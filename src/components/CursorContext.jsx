...
const CursorProvider = ({children}) => {
    ...
    const springConfi = {dumping:20,stiffness:290, mass: 0.45}
  
  const springX = useSpring(mouseX, springConfi)
  const springY = useSpring(mouseY, springConfi)

  const handleMouselMove = (e) => {
  ...
  useEffect(()=>{
    window.addEventListener('mousemove', handleMouselMove)
  return () => window.removeEventListener('mousemove',handleMouselMove)
},[cursor])
